class Sprite
{
  ...
  Sprite(Vector3 pPosition, float pRotation, Vector3 pScale, PImage pTexture, int[] pAnimation, float pAnimationSpeed)
  {
    Vector3[] verts = {new Vector3(-1, 1, 0),
                       new Vector3(1,  1, 0),
                       new Vector3( 1, -1, 0),
    ...
    float[] uv = {0, 1, 1, 1, 0, 0,
     ...
     this.mesh = new Mesh(verts, tris, pTexture, uv, pAnimation, pAnimationSpeed);
     this.mesh.setPosition(pPosition);
     this.mesh.setRotation(new Vector3(0, 0, pRotation));
     this.mesh.setScale(pScale); 
  }
